...
export class AdminService {
   ...
   * @param data Updated reward details.
   */
    // async updateReward(rewardId: number, data: SaveRewardsDto) {
    //     const query = `
    //         UPDATE loyalty_program.tblrewards 
    //         SET reward_title = ?, description = ?, reward = ?, reward_type = ?, reward_price = ?, store_id = ?, region = ?, 
    //         start_date = ?, expiry_date = ?, loyalty_tier = ?, age_group = ?, isActive = ? 
    //         WHERE reward_id = ?
    //     `;
    //     const params = [
    ...
    //     throw new BadRequestException('Unable to update reward');
    //     }
    // }
